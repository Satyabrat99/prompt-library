-- =====================================================
-- FIX DATABASE SCHEMA FOR UPLOAD PROMPT PAGE
-- =====================================================
-- Run these queries in Supabase SQL Editor to fix all schema issues

-- =====================================================
-- 1. CHECK CURRENT PROMPTS TABLE STRUCTURE
-- =====================================================

-- First, let's see what columns exist in the prompts table
SELECT 
    column_name,
    data_type,
    is_nullable,
...
-- Add description column if it doesn't exist (optional)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'Added description column to prompts table';
    ELSE
        RAISE NOTICE 'description column already exists in prompts table';
    END IF;
END $$;

-- =====================================================
-- 3. ENSURE CATEGORIES TABLE HAS REQUIRED COLUMNS
-- =====================================================
...
-- =====================================================
-- 5. UPDATE PROMPTS TABLE COLUMNS TO USE ENUMS
-- =====================================================

-- Update media_type column to use enum
DO $$ 
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'prompts' AND column_name = 'media_type'
        AND data_type != 'USER-DEFINED'
    ) THEN
        ALTER TABLE public.prompts ALTER COLUMN media_type TYPE media_type USING media_type::media_type;
        RAISE NOTICE 'Updated media_type column to use enum';
    ELSE
        RAISE NOTICE 'media_type column already uses enum or does not exist';
    END IF;
END $$;

-- Update difficulty_level column to use enum
DO $$ 
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'prompts' AND column_name = 'difficulty_level'
        AND data_type != 'USER-DEFINED'
    ) THEN
        ALTER TABLE public.prompts ALTER COLUMN difficulty_level TYPE difficulty_level USING difficulty_level::difficulty_level;
        RAISE NOTICE 'Updated difficulty_level column to use enum';
    ELSE
        RAISE NOTICE 'difficulty_level column already uses enum or does not exist';
    END IF;
END $$;

-- =====================================================
-- 6. ENSURE USER_PROFILES TABLE EXISTS WITH ROLE COLUMN
-- =====================================================
...
-- =====================================================
-- 11. ENSURE UNIQUE CONSTRAINT ON CATEGORIES NAME
-- =====================================================
...
INSERT INTO public.categories (name, description, slug, color) VALUES 
    ('AI Art', 'Artificial Intelligence generated artwork and images', 'ai-art', '#FF6B6B'),
    ('Photography', 'Photography prompts and techniques', 'photography', '#4ECDC4'),
    ('Design', 'Graphic design and visual composition', 'design', '#45B7D1'),
    ('Writing', 'Creative writing and storytelling', 'writing', '#96CEB4'),
    ('Marketing', 'Marketing and advertising content', 'marketing', '#FFEAA7'),
    ('Education', 'Educational and learning materials', 'education', '#DDA0DD'),
    ('Technology', 'Tech-related prompts and tutorials', 'technology', '#98D8C8'),
    ('Business', 'Business and professional content', 'business', '#F7DC6F')
ON CONFLICT (name) DO NOTHING;

-- =====================================================
-- 13. CREATE USER REGISTRATION HANDLER
-- =====================================================
...
-- =====================================================
-- 14. CREATE ADMIN PROMOTION FUNCTION
-- =====================================================
